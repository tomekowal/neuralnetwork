...
object XOR extends App {
    ...
    val weights = new FileParserNeuralNetwork("bpxor1.txt").weightsFromFile
    val nn = new NeuralNetwork(weights)
    nn.randomize(0.0, 0.2)
    val training = List( ( List(0.0,0.0), List(0.0)),
    		      	 ( List(1.0,0.0), List(1.0)),
			 ( List(0.0,1.0), List(1.0)),
			 ( List(1.0,1.0), List(0.0)) )

    val teacher = new BackPropagationTeacher(0.5, 0.5, 10000)
    val iterations = 1
